...
class JPEGProcessor:
    ...
    def encode_pipeline(self, image):
        ...
        # Lưu shape
        shape = blocks.shape[:-2] if blocks.ndim == 4 else blocks.shape[:3]
    ...
    def decode_pipeline(self, encoded_data, dc_codes, ac_codes, shape):
        """
        ...
        rle_data = huffman_decode_bitstring(encoded_data, dc_codes, ac_codes)
        self.intermediates['rle'] = rle_data
        
        # Bước 2: Giải RLE và zigzag
        quant_blocks = apply_inverse_zigzag_and_rle(rle_data, shape)
        self.intermediates['quantized'] = quant_blocks.copy()
        ...
        # Bước 5: Gộp khối
        image = merge_blocks(pixel_blocks)
